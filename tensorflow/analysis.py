...
def train():
    ...
    model_file = '/data/lmkhi/work/project/HN-NPS-RNN/HN-NPS-RNN/tensorflow/model/libri_build_hn_npsrnn_bidirectional_feed_concat_multilayer_n1400_l30_r30_nodrop_sgd_start0_1/model-5'
    ...
    with tf.device(device_id):
        if True:
            ...
            hiddenlist = [1400] * config.N_INPUT_FRAME
            ...
            b_num_examples, sf = classifier.analysis(sess, softmax, pl_images, pl_labels, pl_keep_prob,
                                                                      test_image_manager.images,
                                                                      test_label_manager.images,
                                                                      batch_size, device_id)

            # sfmax = np.amax(sf, axis=1)
            print (np.mean(sf))
            print (np.std(sf))
            print (sf < 0.5).sum()
            print (sf > 0.9).sum()
