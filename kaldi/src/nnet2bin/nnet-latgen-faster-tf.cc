// nnet2bin/nnet-latgen-faster-tf.cc
...
#include "nnet2/decodable-am-tf.h"
...
int main(int argc, char *argv[]) {
  try {
    ...
    std::string model_in_filename = po.GetArg(1),
        ...
        tfserving_ip_address = po.GetArg(5),
        left_context = po.GetArg(6),
        right_context = po.GetArg(7),
        words_wspecifier = po.GetOptArg(8),
        alignment_wspecifier = po.GetOptArg(9);

     int32 lcontext = std::stoi(left_context);
     int32 rcontext = std::stoi(right_context);
    ...
    if (ClassifyRspecifier(fst_in_str, NULL, NULL) == kNoRspecifier) {
...

      {
        ...
        for (; !feature_reader.Done(); feature_reader.Next()) {
          ...
          bool pad_input = true;
          DecodableAmTf nnet_decodable(trans_model,
                                         am_nnet,
                                         features,
                                         pad_input,
                                         acoustic_scale,
